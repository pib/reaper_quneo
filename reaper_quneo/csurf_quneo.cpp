...
public:
  CSurf_Quneo(int offset, int size, int indev, int outdev, int *errStats)
  {
    m_offset=offset;
    m_size=size;
    m_midi_in_dev=indev;
...
static IReaperControlSurface *createFunc(const char *type_string, const char *configString, int *errStats)
{
  ...
  return new CSurf_Quneo(parms[0],parms[1],parms[2],parms[3],errStats);
}
